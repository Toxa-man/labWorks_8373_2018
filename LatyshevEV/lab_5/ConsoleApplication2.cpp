...
void difference(int* matrix1, int* matrix2, int* result_matrix, int length1, int length2, int height1, int height2)
{
	for (int i = 0; i < length1; i++)
	{
		for (int k = 0; k < height1; k++)
...
int main()
{
	...
	while (flag == false)
	{
		...
		case ('s'):  
			...
			{
				summ((int*)matrix1, (int*)matrix2, (int*)result_matrix, length1, length2, height1, height2);
		...
		case ('d'): 
			if (length1 != length2 || height1 != height2)
			{
				...
				difference((int*)matrix1, (int*)matrix2, (int*)result_matrix, length1, length2, height1, height2);
				my_print((int*)result_matrix, length2, height1);
			}
			break;
		case ('m'): 
			...
			{
				multiplication((int*)matrix1, (int*)matrix2, (int*)result_matrix, length1, length2, height1, height2);
				my_print((int*)result_matrix, length2, height1);
			}
			break;
		case('q'): flag = true;
